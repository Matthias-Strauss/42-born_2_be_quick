...
echo -e "Available Memory & Utilization: $(free -h | awk '/^Swap/ {print $4 "/" $2 " (" int($3/$2*100) "%)"}')
...
"
    ...
    ascii_lines=$(echo -n "${logo}" | wc -l)

    # Print ASCII art and content with the box outline
    for ((i = 1; i <= $ascii_lines; i++)); do
        printf '%s' "$(echo "${logo}" | sed -n "${i}p")"
        printf '%*s' $((content_length + 5 - ${#logo})) "$(echo "${content}" | sed -n "${i}p")"
        printf '\n'
    done
